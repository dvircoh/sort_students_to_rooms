...
public:
	...
	virtual std::vector<std::string> get_students_names() = 0;
	virtual Student get_student(const std::string& name) = 0;

	//room method
	virtual void add_room(int room_id, int room_size) = 0;
	virtual std::vector<Room> get_rooms() = 0;
	virtual void populate_rooms(std::vector<Room>& rooms) = 0;
};
